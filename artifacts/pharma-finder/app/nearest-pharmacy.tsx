    ...
    try {
      ...
      if (status !== "granted") {
        Alert.alert(
          ...
          isRTL
            ? "يُرجى السماح بالوصول إلى الموقع"
  ...
  const openMaps = (item: NearestPharmacy) => {
    ...
    if (item.lat && item.lon) {
      ...
      const query = encodeURIComponent(
        (item.addressAr || item.address) + ", Mauritanie",
      );
      url =
        Platform.OS === "ios"
          ? `maps:?q=${query}`
      ...
      <View style={[styles.cardHeader, isRTL && styles.rtlRow]}>
        ...
        <View style={[styles.cardInfo, isRTL && { alignItems: "flex-end" }]}>
          ...
          <View style={[styles.addressRow, isRTL && styles.rtlRow]}>
            ...
            />
            <Text
              style={[styles.addressText, isRTL && styles.rtlText]}
              numberOfLines={2}
            >
  ...
  return (
    ...
    >
      <View style={[styles.header, isRTL && styles.rtlRow]}>
        ...
        <View
          style={[styles.headerTitleWrap, isRTL && { alignItems: "flex-end" }]}
        >
        ...
        <View style={styles.headerActions}>
          ...
          <TouchableOpacity
            style={[
              styles.gpsBtn,
              showMap && { backgroundColor: Colors.primary + "20" },
            ]}
            onPress={() => {
              Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
              setShowMap((v) => !v);
